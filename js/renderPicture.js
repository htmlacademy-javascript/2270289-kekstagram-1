...
function renderingPictureUsers() {
  ...
  usersList.forEach(({url,likes,comments,id}) => {
    ...
    userPictureImage.setAttribute('data-Id-UserList',id); // Добавим атрибут для связи с идентификатором списка пользователей
  ...
  const onClickPicture = function (evt) {
    if (evt.target.matches('.picture__img')) {
      openBigPicture(evt.target, usersList);
    }
